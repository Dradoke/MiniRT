...
{
	cam->transform[0][0] = right.x;
	cam->transform[1][0] = right.y;
	cam->transform[2][0] = right.z;
	cam->transform[0][1] = up.x;
	cam->transform[1][1] = up.y;
	cam->transform[2][1] = up.z;
	cam->transform[0][2] = fwd.x;
	cam->transform[1][2] = fwd.y;
	cam->transform[2][2] = fwd.z;
}
...
static void	handle_mouse_rotation(t_data *data)
{
	...
	r = (t_vec3){{data->scene.cam.transform[0][0],
		data->scene.cam.transform[1][0], data->scene.cam.transform[2][0]}};
	u = (t_vec3){{data->scene.cam.transform[0][1],
		data->scene.cam.transform[1][1], data->scene.cam.transform[2][1]}};
	f = (t_vec3){{data->scene.cam.transform[0][2],
		data->scene.cam.transform[1][2], data->scene.cam.transform[2][2]}};
...
static void	handle_standing_movement(t_data *data)
{
	...
	move = (t_vec3){{0, 0, 0}};
	forward_ground = (t_vec3){{data->scene.cam.transform[0][2], 0,
		data->scene.cam.transform[2][2]}};
	...
	if (data->keys[FTKEY_A])
		move = vec3_add(move, vec3_scale(right_ground, -MOVE_SPEED));
	data->scene.cam.transform[0][3] += move.x;
	data->scene.cam.transform[1][3] += move.y;
	data->scene.cam.transform[2][3] += move.z;
}
